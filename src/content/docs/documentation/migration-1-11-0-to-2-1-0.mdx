...
You can find the all the new additions compiled in the [**2.1.0** release page](/about/version-2-1-0). Going through the changelog may improve your understanding of the migration / update process.
...
In order to migrate other SQL backends, the complete list of structural changes may be found below. However, if possible, we recommend re-creating the databases from scratch using the [DB deployment tutorial](/documentation/install-dbdeployment-2-1).
...
* All **tcp_xxx** and **tls_xxx** core parameters have been moved to their respective modules ([proto_tcp](http://www.opensips.org/html/docs/modules/2.1.x/proto_tcp.html), respectively [proto_tls](http://www.opensips.org/html/docs/modules/2.1.x/proto_tls.html))
