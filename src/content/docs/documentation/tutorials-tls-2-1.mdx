...
We will need to specify the TLS method which will specify what type of protocol we will use. In this tutorial we will use “TLSv1”, but you can always use another one, see the full list in the **[module’s documentation section](http://www.opensips.org/html/docs/modules/2.1.x/proto_tls.html#id294154)**. 

```c
modparam("proto_tls", "tls_method", "TLSv1")
```

Also, in order for OpenSIPS to start, we need to configure some "global" certificates. We need to do this because we need a default match, if no domain is matched, we will need a default case, something to match everything, just like **0.0.0.0** route in routing tables. You can always jump over the following section [**3.4 Setting up TLS domains**](/documentation/tutorials-tls-2-1#toc12) directly to [**3.5 Full script example**](/documentation/tutorials-tls-2-1#toc12), because they represent a more specific type of scripting, but it is recommended to read about **server domains** which allow you to use different settings on different listening interfaces. 
...
As specified in section [**3.1 Overview**](http://www.opensips.org/Documentation/Tutorials-TLS-2-1?action=edit#toc8), our scenario includes two TLS connections, one from the UAC to OpenSIPS and the second one from OpenSIPS to the UAS. Whereas in the first connection OpenSIPS will be the server side of the connection, in the second one it will be the client side so we need to define two different TLS domains.
