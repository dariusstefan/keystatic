...
subtitleHref: "http://www.opensips.org/html/docs/modules/4.0.x/proto_ws.html"
...
Also important to note that as of 4.0, certificate management has been split out into a new module, [TLS_MGM](http://www.opensips.org/html/docs/modules/4.0.x/tls_mgm.html).  Setting appropriate modparams for the tls_mgm module is how we will manage our certificates for both WSS and TLS.
...
* `-n`: the listening IP and port that is used by OpenSIPS to communicate with the RTPengine
...
In order to use WebSocket and WebSocketSecure in OpenSIPS, one has to load the [proto_ws](http://www.opensips.org/html/docs/modules/4.0.x/proto_ws) and [proto_wss](http://www.opensips.org/html/docs/modules/4.0.x/proto_wss) into its configuration file and define a listener for the WebSocket and WebSocketSecure protocol.  We also must load the [tls_mgm](http://www.opensips.org/html/docs/modules/4.0.x/tls_mgm) module in order to manage our certificates.
...
Next, the [rtpengine](http://www.opensips.org/html/docs/modules/4.0.x/rtpengine) module has to be loaded and configured to communicate with the `rtpengine` daemon.
```c
...
Note that the [rtpengine_sock](http://www.opensips.org/html/docs/modules/4.0.x/rtpengine#rtpengine.p.rtpengine_sock) parameter should be the same as the `-n` parameter sent to the `rtpengine` daemon, and OpenSIPS should have IP connectivity to that socket.
...
In case the request is a REGISTER, we want to store this information in the *location* table, so that we know then the user is called. To do that, we can set a branch flag before calling the [save()](http://www.opensips.org/html/docs/modules/4.0.x/registrar.html#id294034) function. This way, when the [lookup()](http://www.opensips.org/html/docs/modules/4.0.x/registrar.html#id294366) method returns, we will be able to determine whether the client uses WebSocket or not.
...
When a call is placed, based on the two flags (`STR_WS` and `DST_WS`) we can determine what our caller and callee can "speak" (either RTP or DTLS-SRTP) and instruct the `rtpengine` daemon how to handle the call. We can do that by tuning the parameters passed to the  [rtpengine_offer()](http://www.opensips.org/html/docs/modules/4.0.x/rtpengine#rtpengine.f.rtpengine_offer) function.
```text
...
The [rtpengine_answer()](http://www.opensips.org/html/docs/modules/4.0.x/rtpengine#rtpengine.f.rtpengine_answer) function logic should look like this:
...
Now, all we have to do is to close the RTP/SRTP session when the call is ended. To do that, we use the [rtpengine_delete()](http://www.opensips.org/html/docs/modules/4.0.x/rtpengine#rtpengine.f.rtpengine_delete) function:
