...
Instead of full-mesh replicating user location data to the entire cluster (i.e. just like the ["full sharing"](/documentation/tutorials-distributed-user-location-full-sharing) clustering strategy does), the nodes will only publish some light, metadata "AoR availability" records into a shared NoSQL database, as in this picture:
...
We need not change anything in the default script in order to achieve the above. The [working_mode_preset](http://www.opensips.org/html/docs/modules/2.4.x/usrloc.html#param_working_mode_preset) or [cluster_mode](http://www.opensips.org/html/docs/modules/2.4.x/usrloc.html#param_cluster_mode) module parameters take care of this.
...
* local lookup, using [lookup("location")](http://www.opensips.org/html/docs/modules/2.4.x/registrar#func_lookup)
* global lookup, using [lookup("location", "g")](http://www.opensips.org/html/docs/modules/2.4.x/registrar#func_lookup)
...
At OpenSIPS script level, we can easily distinguish calls coming in from fellow cluster nodes using [cluster_check_addr()](http://www.opensips.org/html/docs/modules/2.4.x/clusterer#func_cluster_check_addr):
...
We then enable these branch flags for some or all contacts before calling [save()](http://www.opensips.org/html/docs/modules/2.4.x/registrar#func_save):
...
To silence these errors, you can hook the [nh_enable_ping](http://www.opensips.org/html/docs/modules/2.4.x/nathelper.html#mi_nh_enable_ping) MI command into your active->backup and backup->active transitions of the VIP:
