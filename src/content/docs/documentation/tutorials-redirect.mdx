...
You will also notice in this example the use of the function [exec_dest()](http://www.opensips.org/html/docs/modules/1.4.x/exec.html#id228172). This function calls an external script and assumes that the script returns a SIP URI. Then sets the "Contact" header to the returned SIP URI. You can also use pseudo variables in the exec_dset() function. In this example `$tU` (To Username) and `$fU` (From Username) are used.
...
> **Observation:** To get this to work correctly with Asterisk you need to add "promiscredir=yes" to the general section of your sip.conf
