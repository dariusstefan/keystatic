...
There are no special helper functions for this purpose, so such operations will have to be done with a *[remove_hf()](https://opensips.org/docs/modules/3.3.x/sipmsgops.html#func_remove_hf)* function call, followed by an *[append_hf()](https://opensips.org/docs/modules/3.3.x/sipmsgops.html#func_append_hf)*.  For example, say we want to remove support for RCS File Transfer (our platform does not have it), while always advertising support for RCS Geolocation PUSH (since we can handle those types of payloads).
