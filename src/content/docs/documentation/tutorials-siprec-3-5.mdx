...
subtitleHref: "http://www.opensips.org/html/docs/modules/3.5.x/siprec.html"
...
The work for the [SIPREC](http://www.opensips.org/html/docs/modules/3.5.x/siprec.html) module has been sponsored by the [OrecX Company](http://www.orecx.com/).
...
We will start from an existing working setup for OpenSIPS and gradually add the necessary parts to it until we end up with a working Call Recording script. We will use **OpenSIPS 3.5** [default script](https://github.com/OpenSIPS/opensips/blob/master/etc/opensips.cfg), which provides basic call routing for clients.
...
By default, fail-over is driven by a negative response from the SRS, or by an auto-generated 408 if the SRS does not respond. However, not all responses mean the service is unavailable, some of them might simply indicate that the call should not recorded. In this case, you might want to ignore some response codes from the fail-over algorithm. This is done using the [skip_failover_codes](http://www.opensips.org/html/docs/modules/3.5.x/siprec#skip_failover_codes) parameter. The following example prevents fail-over for any class 3 and 4 response codes - thus it only fails-over 5xx and 6xx classes:
