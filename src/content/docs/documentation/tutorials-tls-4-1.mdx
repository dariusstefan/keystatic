...
We will need to specify the TLS method which will specify what type of protocol we will use. In this tutorial we will use “TLSv1”, but you can always use another one, see the full list in the **[module’s documentation section](http://www.opensips.org/html/docs/modules/4.1.x/proto_tls.html#id294154)**. 
...
As specified in section [**4.1 Overview**](http://www.opensips.org/Documentation/Tutorials-TLS-4-1?action=edit#toc8), our scenario includes two TLS connections, one from the UAC to OpenSIPS and the second one from OpenSIPS to the UAS. Whereas in the first connection OpenSIPS will be the server side of the connection, in the second one it will be the client side so we need to define two different TLS domains.
