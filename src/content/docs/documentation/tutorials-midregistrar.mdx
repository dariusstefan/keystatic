...
For all registrations, we make sure to call *mid_registrar_save()*. As stated in its [documentation](http://www.opensips.org/html/docs/modules/2.3.x/mid_registrar.html), it will not process the contacts immediately, but rather do the necessary changes to the REGISTER request before it is relayed to the main registrar, while also preparing to transparently handle its replies. The above code stays the same regardless of the module's working mode or insertion mode. We are now done with REGISTER request handling!
