...
* [event_datagram](http://www.opensips.org/html/docs/modules/1.8.x/event_datagram) - sends Datagrams over UDP or UNIX sockets
* [event_rabbitmq](http://www.opensips.org/html/docs/modules/1.8.x/event_rabbitmq) - sends an AMQP message to a RabbitMQ server
...
An external application should subscribe for an event using MI commands when the subscription nature is very dynamic: it subscribes for a short period, receives notifications, then decides to unsubscribe and so on. The MI function used to subscribe for an event is [event_subscribe](/documentation/interface-coremi-1-8#toc15).
...
The Event Interface allows the script writer to subscribe for an event directly from the OpenSIPS configuration script. This is usually used when a subscription is permanent, that never expires. For example a server that is continuously listening from events from OpenSIPS and process them accordingly. The command used to subscribe for an event from script is [subscribe_event](/documentation/script-corefunctions-1-8#toc154).
...
If the script writer wants the subscription to expire after 1200 second, the third parameter of the [subscribe_event](/documentation/script-corefunctions-1-8#toc154) function can be specified:
...
* [E_PIKE_BLOCKED](http://www.opensips.org/html/docs/modules/1.8.x/pike#id250185) - raised when **pike** module decides an IP should be blocked.
* [E_RTPPROXY_STATUS](http://www.opensips.org/html/docs/modules/1.8.x/rtpproxy#id293504) - raised when a RTPProxy connects or disconnects.
* [E_DISPATCHER_STATUS](http://www.opensips.org/html/docs/modules/1.8.x/dispatcher#id293668) - raised by the **dispatcher** module when a destination address becomes active/inactive.
* [E_CORE_THRESHOLD](/documentation/interface-coreevents-1-8#toc1) - raised when using debugging bottleneck detection and the limit is exceeded.
...
In order to support this type of communication, the [event_datagram](http://www.opensips.org/html/docs/modules/1.8.x/event_datagram) module has to be loaded. Consult the module's documentation page for more information about subscription socket and encapsulation format.
...
The [event_rabbitmq](http://www.opensips.org/html/docs/modules/1.8.x/event_rabbitmq) module provides this functionality. Note that in this case, the "external application" is the RabbitMQ server, so you do not have the possibility to subscribe for events from it. Therefore the programmer should either use a different application to subscribe for the event, or subscribes for it directly from the OpenSIPS script (as discussed [here](/documentation/tutorials-eventinterface-1-8#toc5)). For more information please visit the module's documentation page.
