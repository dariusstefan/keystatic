---
title: "Migration from 1.11.x to 2.3.0"
description: "This section is meant to provide useful help in migrating OpenSIPS installations from the 1.11 version to the latest 2.3 release."
---

This section is meant to provide useful help in migrating OpenSIPS installations from the **1.11** version to the latest **2.3** release.

You can find the all the new additions compiled in the [**2.3.0** release page](/about/version-2-3-0). Going through the changelog may improve your understanding of the migration / update process.

---

## DB migration
For MySQL users, you can easily migrate your 1.11.x OpenSIPS databases to the 2.3.0 format with the **opensipsdbctl** tool:
```text
...
* *opensips_2_1* is the DB name to be created for 2.3 format
...
In order to migrate other SQL backends, the complete list of structural changes may be found below. However, if possible, we recommend re-creating the databases from scratch using the [DB deployment tutorial](/documentation/install-dbdeployment-2-3).
...
* All **tcp_xxx** and **tls_xxx** core parameters have been moved to their respective modules ([proto_tcp](http://www.opensips.org/html/docs/modules/2.3.x/proto_tcp.html), respectively [proto_tls](http://www.opensips.org/html/docs/modules/2.3.x/proto_tls.html))
