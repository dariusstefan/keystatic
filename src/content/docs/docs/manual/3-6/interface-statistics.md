...
All available statistic variables are listed and documented : statistics provided [by OpenSIPS core](/manual/3-6/interface-corestatistics) or by [OpenSIPS modules](/manual/3-6/modules) (see the Statistics chapter for each module).

---

## Usage

To get access to the statistics you have to use the [MI interface](/manual/3-6/interface-mi) which provides (directly from OpenSIPS core) several MI functions for:  
\
**Fetching the value** of a statistic variable, of an entire group of variables or of all variables. The MI [get_statistics](/manual/3-6/interface-coremi) command can be used here:
```bash
...
> Reseting a computed-value statistic will be ignored and have no effect.
 ...
 The MI [reset_statistics](/manual/3-6/interface-coremi) command can be used here:
