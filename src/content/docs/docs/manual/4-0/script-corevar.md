...
* **transformation** - a series of processing actions can be applied on pseudo-variable. You can find the whole list of possible transformations [here](/manual/4-0/script-tran). The transformations can be cascaded, using the output of one transformation as the input of another.     
...
`$log_level` - changes the log level for the current process ; the log level can be set to a new value (see [possible values](/manual/4-0/script-coreparameters#log_level) or it can be reset back to the global log level.
...
For more details on the meaning of these sub-fields, please also read about the [socket definition](/manual/4-0/script-coreparameters#toc66).
