...
description: "Overview of OpenSIPS core variable types, syntax, usage, and behavior in routing scripts."
---
...
$(<span class="color-green"><em>&lt;context&gt;</em><strong>name</strong><em>(subname)[index]\{transformation\}</em></span>)
...
Usage examples:
- Only **name**: $ru 
- **Name** and *'subname*: <span class="color-green">$hdr(Contact)</span>
- **Name** and **index**: <span class="color-green">$(ct[0])</span>
- **Name**, **subname** and **index**: $(avp(i:10)[2])
- **Context** 
  - <span class="color-green">$(&lt;request&gt;ru)</span> from a reply route will get the Request-URI from the request
  - <span class="color-green">$(&lt;reply&gt;hdr(Contact))</span> context can be used from failure route to access information from the reply 
...
<Aside type="caution">TO BECOME OBSOLETE</Aside>

, replaced by [$msg.branch.flags](#msg.branch.flags)
...
, replaced by [$msg.branch.uri](#msg.branch.uri)
Examples:
```c
...
<Badge text="It is R/W variable (you can assign values to it from routing logic)" variant="caution" />
...
, replaced by [$msg.branch.flag](#msg.branch.flag)
...
<Badge text="It is R/W variable (you can assign values to it routing script)" variant="caution" />
