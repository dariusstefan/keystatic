...
* **Core events** - internal events that trigger changes of OpenSIPS core/global behavior. A full list of exported core events can be found [here](/manual/4-0/interface-coreevents).
* **Modules events** - events triggered by each module, when loaded. Each module can export zero, one or more events. Details can be found in the [documentation page](/manual/4-0/modules) of each module.
* **Custom events** - triggered from script using the [raise_event()](/manual/4-0/script-corefunctions#raise_event) command.
...
You can subscribe for an event either at startup (using the [subscribe_event()](/manual/4-0/script-corefunctions#subscribe_event) command in the script) or during runtime, using the [event_subscribe](/manual/4-0/interface-coremi#event_subscribe) MI command.

---

## Examples

In order to configure a RabbbitMQ server to be notified when a custom event is triggered, first you have to subscribe it to the event, using the [subscribe_event()](/manual/4-0/script-corefunctions#subscribe_event) command:
...
Then, in order to trigger the event from the script, call the [raise_event()](/manual/4-0/script-corefunctions#raise_event) command when needed:
